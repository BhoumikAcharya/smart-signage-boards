...
const int BATTERY_PIN = 34;       // The GPIO pin connected to the voltage divider
...
void loop() {
  ...
  float k = 1.779; //calibration facotr added according to the current values --- 07/04/2026
  float battVoltage = pinVoltage * ((R1 + R2) / R2) ;

  // 4. Calculate Battery Percentage
  float percentage = ((battVoltage - BATT_MIN_V) / (BATT_MAX_V - BATT_MIN_V)) * 100.0;
  
  // Constrain the percentage between 0% and 100%
  //remocing the percentage function for now ---- 07/04/2026 ------- bhoumik
  ...
  Serial.print("State of Charge:"); Serial.print(percentage, 1); Serial.println(" %");
