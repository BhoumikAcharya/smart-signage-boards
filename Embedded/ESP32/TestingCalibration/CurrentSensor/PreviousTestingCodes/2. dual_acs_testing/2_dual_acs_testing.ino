...
const float SENSOR_1_THRESH_OK = 0.27;      // Above this is OK

// Sensor 2 Thresholds
const float SENSOR_2_THRESH_PARTIAL = 0.08; // Below this is FAIL
const float SENSOR_2_THRESH_OK = 0.25;      // Above this is OK
...
String determineCurrentStatus(int pin, int zeroPoint, float sensitivity, float threshOK, float threshPartial, float &outRaw, float &outAmps) {
  ...
  // MODIFIED: Use specific calibration values for calculation
  float currentAmps = abs(avgRaw - zeroPoint) / sensitivity;
