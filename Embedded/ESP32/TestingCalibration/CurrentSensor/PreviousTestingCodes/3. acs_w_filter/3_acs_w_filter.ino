...
String determineCurrentStatus(int pin, float offsetVoltage, float &outRaw, float &outAmps) {
  // --- MODIFIED: Low Pass Filter (Exponential Moving Average) ---
  float filteredRaw = analogRead(pin); // Initialize with first reading
  float alpha = 0.1; // Smoothing Factor (0.1 = heavy smoothing, 1.0 = no smoothing)
  
  // Take 100 samples
  for(int i=0; i<30; i++) {
    int raw = analogRead(pin);
    // EMA Formula: NewFiltered = (alpha * Raw) + ((1-alpha) * OldFiltered)
    filteredRaw = (alpha * raw) + ((1.0 - alpha) * filteredRaw);
    delay(25); // 0.5ms delay between reads (Total ~50ms)
  }
  
  // Use the filtered value for calculations
  outRaw = filteredRaw;
  ...
  float pinVoltage = (filteredRaw / ADC_RESOLUTION) * ADC_REF_VOLTAGE;
  float currentAmps = (pinVoltage - offsetVoltage) / ACS_SENSITIVITY;
  
  // Negative Handling: If voltage dropped below offset (negative current), force to 0.
  if (currentAmps < 0) {
    currentAmps = 0.0;
  }
...
void checkSensors() {
  ...
  if (millis() - lastCurrentCheck > 2000) { 
    ...
    // --- DEBUG PRINTS ---
    Serial.println("--- Sensor Readings (Filtered) ---");
    Serial.printf("Power Monitor (Pin 34): Raw ADC = %d\n", analogValue);
    Serial.printf("Load 1 (Pin 35): Raw=%.0f, Amps=%.3f A, Status=%s\n", 
                  raw1, amps1, status1.c_str());
    Serial.printf("Load 2 (Pin 36): Raw=%.0f, Amps=%.3f A, Status=%s\n", 
