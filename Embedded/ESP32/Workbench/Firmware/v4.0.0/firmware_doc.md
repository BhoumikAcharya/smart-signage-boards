...
**Baseline:** forked from `ESP32/Firmware/Firmware.ino` v3.0.0. See [`../../../Firmware/Frimware_v3.0.0/firmware_doc.md`](../../../Firmware/Frimware_v3.0.0/firmware_doc.md) for the full architecture reference — this document only covers what is **different** in the Workbench build.
...
- Source of truth for the pattern/rationale: [`../../Firmware_Test/demo_mode_v2.0.0/CHASE_ANIMATION_SPEC.md`](../../Firmware_Test/demo_mode_v2.0.0/CHASE_ANIMATION_SPEC.md). Note the production firmware is **command-driven** (mode 1 = LHS chase, 2 = RHS, 3 = both), so it does **not** implement that spec's bench-only phase loop (LEFT/RIGHT/OFF auto-cycling).
...
Carried over from v3.0.0 without modification — refer to the [baseline doc](../../../Firmware/Frimware_v3.0.0/firmware_doc.md) for detail:
